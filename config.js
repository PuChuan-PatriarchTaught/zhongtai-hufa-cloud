 ...
 *   第一次呼叫時服務需要重新啟動（類似「嗚醒」的過程），偶爾會
 *   來不及回應或連線失敗，屬於 Apps Script 平台本身的特性，難以
 *   完全避免。修正方式：改成「連線失敗時，自動惄惄重試一次」，
...
async function 呼叫後端API(obj請求內容) {
  var int最大單試次數 = 2; // 第一次失敗後，自動再重試一次，總共最多單試 2 次

  for (var int第幾次單試 = 1; int第幾次單試 <= int最大單試次數; int第幾次單試++) {
    try {
      ...
      // 或 Apps Script Web App 閒置一段時間後第一次呼叫的嗚醒延遲
      console.error("呼叫後端 API 發生錯誤（第 " + int第幾次單試 + " 次單試）：", 錯誤);

      if (int第幾次單試 < int最大單試次數) {
        // 惄惄等待片刻後自動重試一次，不打擾操作人員
 ...
 * 函式：取得目前登入精舍資訊
 * 版本：v1.0（2026-09-16）
 * 用途：從 sessionStorage 讀出登入狀態，統一由這個函式
